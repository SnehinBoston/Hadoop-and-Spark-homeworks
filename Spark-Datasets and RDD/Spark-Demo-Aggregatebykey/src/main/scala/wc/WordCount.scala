...
object WordCountMain {
  def rdd_ak(textFile: RDD[String],outPath: String): Unit ={
    val title = textFile.map(line => line.split(",")(1)).map(word =>(word,1))
    // Aggregate by key
    val aggregate_key_output = title.aggregateByKey(0)(_+_,_+_)
    aggregate_key_output.saveAsTextFile(outPath)
    println(aggregate_key_output.toDebugString)
  }
