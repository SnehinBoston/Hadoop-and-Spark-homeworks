...
1. First, define these four functions as the trivial functions (do nothing).
  ...
  a. Observe that you hit an assert statement as readers and writers conflict.
     Explain why.
     Because the variable num_readers has already been updated before the write thread is called, that is the read thread runs first, updates num_readers
  ...
  b. Now let's look at the running time, if we ignore the assert statement.  Do:
     ...
     (HINT:  If you look closely at read_write_locks.c, you'll see that there
      ...
      There is actually no synchronization here, so reader and writer threads work parallel that is when one thread sleeps, other thread acquires the task_stats lock and completes the task.
...
- So, there are 20 read tasks and 20 write tasks being done over here.
...
3. Next, let's see if we can produce a more efficient version of this solution.
  ...
  a. With the above design, do:  make case3
     ...
     The code will run for a very long time because this is kind of a sequential execution the very first write thread acquires the write lock,
  ...
  b. Now, modify acquire_exclusive_lock() by moving the final
     ...
     In this mechanism, while the first thread is executing(after acquiring the lock), the second thread acquires the acquire_release_lock,
...
4. We saw that there was a bug in Case 3.a, when NUM_READERS is set to 2.
  ...
  a. Then do:  make case4
     Explain what happens, and why.
     For the shared read lock, the read thread gets the acquire_release_lock and if the write lock is held,
     ...
     20 reads and 20 writes in 352 milliseconds.

  b. Next, modify read_write_locks.c to set NUM_READERS to 5.
     Do:  make case4
     Explain how the printed statistics change and why.
     Changing the number of readers has very little impact on the running time as multiple read threads can read at a time,
     and also read threads do not sleep waiting to acquire the lock.
     50 reads and 20 writes in 372 milliseconds.

  c. Next, also modify read_write_locks.c to set NUM_WRITERS to 5.
     Do:  make case4
     Explain how the printed statistics change and why.
     50 reads and 50 writes in 703 milliseconds.
